...
### 日历 & 日记
- **日历视图**：月度打卡一目了然，状态色条 + 圆点 + 天气心情图标
- **日记功能**：每天可写日记 + 记录工作任务，有日记的日期盖红色"日记"印章
- **图片支持**：Ctrl+V 粘贴图片，自动转存 PNG 到 Obsidian assets 文件夹
- **Obsidian 联动**：日记双向同步为 `.md` 文件，YAML frontmatter + 正文
...
# 含 Obsidian 同步（需先 pip install -r backend/requirements.txt）
# 终端 1：启动后端
npm run dev:backend
# 终端 2：启动前端
npm run dev
```
访问 http://localhost:5173。前端 :5173，后端 :8765。
...
│   ├── App.vue               # 根组件
│   ├── style.css             # 设计令牌 + 全局样式
│   └── theme.css             # Element Plus 主题覆盖（靛蓝 #6366f1）
...
### 2. 日历视图 (`/calendar`)
- 月度日历网格，支持月份切换和回到今天
- 日期格子精简设计：左上角日期数字、右上角打卡状态圆点、右下角天气/心情图标
- 有日记的日期显示红色半透明"日记"印章（旋转斜盖）
- "今天"以淡紫渐变背景突出
- 点击日期查看详情面板，支持天气和心情 emoji 选择
- **日记功能**：
  - 工作任务输入 + 富文本编辑区（支持 Ctrl+V 粘贴图片）
  - **Obsidian 同步**：保存时自动写入 `.md` 文件到 Obsidian Vault
- 单日和批量补打卡
